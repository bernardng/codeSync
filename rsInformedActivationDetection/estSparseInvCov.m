...
nLevels = 5; % Number of refinements on lambda grid
parfor sub = 1:nSubs
    if dataType == 1 % Load rest data
        load([localpath,'data/imagen/',sublist{sub},'/tcRestParcel1000']);
        X = loadRegressorsRest(sublist{sub}); 
        Y = tcRest;
    elseif dataType == 2 % Load task data
        load([localpath,'data/imagen/',sublist{sub},'/tcTaskParcel1000']);
    ...
    kFolds = 2;
    model = 1;
    K = sparseGGM(Y,paramSelMethod,optMethod,'linear',kFolds,nLevels,nGridPts);
