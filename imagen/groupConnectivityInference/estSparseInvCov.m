...
parfor sub = 1:nSubs
    if dataType == 1 % Load rest data
        tc_parcel = load([localpath,'data/imagen/',sublist{sub},'/restfMRI/tc_rest_fs_parcel500.mat']);
        Y = tc_parcel.tc_parcel;
    elseif dataType == 2 % Load task data
        tc_parcel = load([localpath,'data/imagen/',sublist{sub},'/gcafMRI/tc_task_fs_parcel500.mat']);
        Y = tc_parcel.tc_parcel;
    end
...
for sub = 1:nSubs
    ...
    save([localpath,'data/imagen/',sublist{sub},'/restfMRI/K_rest_fs_parcel500_quic',int2str(kFolds),int2str(nLevels),int2str(nGridPts),'_cv.mat'],'Krest','lambdaBest');
end
