function [x,f,exitflag,output] = logisticRegressionLaplacian(funObj,x0,X,Y,options,varargin)
% [x,f,exitflag,output] = logisticRegressionLaplacian(funObj,x0,options,varargin)
%
...
for i = 1:maxIter
    ...
    [m,n] = size(X);
    h = @(vOpt)(mean(log(1+exp(-Y.*(X*[x(1:end-1),vOpt])))));
    options.numDiff = 1;
    vOpt = minFunc(h,v,options);
    % Compute dual feasible point theta
    plogvOpt = 1./(1+exp(-Y.*(X*[x(1:end-1);vOpt])));
