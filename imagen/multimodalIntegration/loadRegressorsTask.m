...
load([localpath,'data/imagen/',sub,'/gcafMRI/gcaSPM.mat']);
% load(strcat(filepath,sub,'/facesfMRI/facesSPM.mat')); % Face data regressors
regressors = SPM.xX.X;
% Add cosine to account for temporal drifts
nTpts = size(regressors,1); tr = 2.2;
t = (1:tr:tr*nTpts)';
T = tr*nTpts;
freqCut = 1/128; % SPM default is 128s
Tcut = 1/freqCut;
for i = 0:floor(T/Tcut) % 0 => remove DC
