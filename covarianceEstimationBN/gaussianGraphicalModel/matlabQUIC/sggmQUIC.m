...
lineMaxIter = 20; % Maximum #iterations for line search
%Initialization
id = eye(p)==1;
X = X0;
W = X^-1;
...
objNewPrev = inf;
for k = 1:maxIter
    grad = S-W; % Compute grad for finding active set
...
%                 disp(['Newton direction converged in ',int2str(ik),' iterations.']);
    ...
    delT = grad(:)'*D(:)+lambda(:)'*abs(X(:)+D(:))-lambda(:)'*abs(X(:));
...
%         if objNew < objOld+0.001*beta^(ik-1)*(grad(:)'*D(:)+lambda(:)'*abs(X(:)+D(:))-lambda(:)'*abs(X(:)))
        if objNew < objOld+0.001*beta^(ik-1)*delT
